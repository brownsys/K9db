...
// NOTE: Ensure that #batches % #workers = 0
...
DEFINE_uint64(num_workers, 3, "Number of workers");
...
int main(int argc, char **argv) {
  ...
  const uint64_t num_workers = FLAGS_num_workers;
  ...
  pelton::dataflow::Driver driver(type, num_batches, batch_size, num_workers,
                                  num_partitions);
  driver.Execute();
}
