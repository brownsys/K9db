...
bool EqualAsSets(
    const std::vector<RocksdbIndexRecord> &vec1,
    const std::vector<std::pair<rocksdb::Slice, rocksdb::Slice>> &vec2) {
  if (vec1.size() != vec2.size()) {
    return false;
  }
  for (const auto &record : vec1) {
    std::pair<rocksdb::Slice, rocksdb::Slice> pair(record.ExtractShardName(),
                                                   record.ExtractPK());
    if (std::find(vec2.begin(), vec2.end(), pair) == vec2.end()) {
      return false;
    }
  }
  return true;
...
TEST(RocksdbIndexTest, GetAll) {
  ...
  EXPECT_TRUE(EqualAsSets(index.Get({z, o}),
...
TEST(RocksdbIndexTest, SimpleGet) {
  ...
  EXPECT_TRUE(EqualAsSets(index.Get({z}),
                          {{shard1, pk2}, {shard1, pk3}, {shard10, pk6}}));
  ...
  EXPECT_TRUE(EqualAsSets(index.Get({o, t}),
...
TEST(RocksdbIndexTest, GetAfterDelete) {
  ...
  // 2 values at a time.
  EXPECT_TRUE(EqualAsSets(index.Get({t, z}), {{shard1, pk2}, {shard10, pk6}}));

  EXPECT_TRUE(EqualAsSets(index.Get({o, t}), {{shard2, pk4}, {shard10, pk5}}));
