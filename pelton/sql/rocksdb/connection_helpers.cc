...
std::vector<T> RocksdbConnection::GetRecords(
  ...
  if constexpr (DEDUP) {
    lookup = table.IndexLookupDedup(&value_mapper);
  } else {
    lookup = table.IndexLookup(&value_mapper);
  }
  ...
  if (lookup.has_value()) {
    ...
    for (auto it = set.begin(); it != set.end();) {
      ...
      keys.push_back(this->encryption_manager_.EncryptKey(key.Release()));
    }
