...
TEST(RocksdbConnectionTest, SpeedTest) {
  ...
  // do selects
  std::fstream l;
  l.open("pelton/sql/connections/pelton-load.sql", std::ios::in);
  std::vector<
  ...
  while (getline(l, cmd)) {
    count += 1;
    parsed = Parse(cmd);
    selects.push_back({find_pk(cmd), std::move(parsed)});
  }
  auto start = std::chrono::steady_clock::now();
  for (auto &i : selects) {
    conn.ExecuteQuery(i.second.get(), schema, {}, index[i.first]);
  }
