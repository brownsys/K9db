...
CREATE TABLE merit_activity_logs (
    ...
    description text,
    created_at datetime,
    PRIMARY KEY (related_change_id),
    FOREIGN KEY (action_id) OWNED_BY merit_actions(id)
);
...
CREATE TABLE merit_score_points (
    ...
    FOREIGN KEY (score_id) REFERENCES merit_scores(sash_id)
);
