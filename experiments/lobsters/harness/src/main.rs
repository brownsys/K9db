...
const PELTON_SCHEMA: &'static str = include_str!("pelton.sql");
const MARIADB_SCHEMA: &'static str = include_str!("rocks-mariadb.sql");
            ...
            let inner = async move {
                ...
                if let Some(uid) = acting_as {
                    if with_notifications && !priming {
                        match variant {
                            Variant::Pelton => pelton::notifications(c, uid).await,
                        }?;
                    }
                }
