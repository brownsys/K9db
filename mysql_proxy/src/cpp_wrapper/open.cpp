...
CResult *exec_select(ConnectionC *c_conn, const char *query) {
  ...
  if (result.ok() && result.value().IsQuery()) {
    ...
    // allocate memory for CResult struct and the flexible array of union Record
    // pointers (# pointers = # rows/pelton records) according the the number of
    // pelton records
    struct CResult *c_result = (CResult *)malloc(
        sizeof(*c_result) + sizeof(*(c_result->records)) * num_rows);
    ...
    // then allocate memory for the underlying union RecordData that lies behind
    // each RecordData[] pointer (# unions = # of cols)
    for (int r = 0; r < c_result->num_rows; r++) {
      // allocate mem for all the cols of this row (# unions = # cols) | all the
      // union arrays at each ptr
      c_result->records[r] = (CResult::RecordData *)malloc(
          sizeof(*(c_result->records[r])) * c_result->num_cols);
      ...
      for (int j = 0; j < c_result->num_cols; j++) {
        ...
        if (c_result->col_types[j] == ColumnDefinitionTypeEnum::UINT) {
          ...
          std::cout
              ...
              << c_result->records[i][j].UINT << std::endl;
          ...
          std::cout
              << "C-Wrapper: row value for this column in CRecord c_result is: "
              << c_result->records[i][j].INT << std::endl;
          ...
          std::cout
              << "C-Wrapper: row value for this column in CRecord c_result is: "
              << c_result->records[i][j].TEXT << std::endl;
          ...
          std::cout
              << "C-Wrapper: row value for this column in CRecord c_result is: "
              << c_result->records[i][j].DATETIME << std::endl;
        } else {
...
void destroy_select(CResult *c_result) {
  std::cout << "C-Wrapper: starting destroy_select to delete CResult"
            << std::endl;
  // ? I think this results in double free, as rust takes ownership of the
  // strings for (int i = 0; c_result->col_names[i]; i++) {
