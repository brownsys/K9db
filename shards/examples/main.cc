...
int main(int argc, char **argv) {
  ...
  shards::exec(&state, "SET echo;", &Callback, nullptr, nullptr);

  // Create all the tables.
  std::cout << "Create the tables ... " << std::endl;
  for (std::string &create : CREATES) {
    std::cout << std::endl;
    shards::exec(&state, create, &Callback, nullptr, nullptr);
  }
  std::cout << std::endl;
  ...
  for (const auto &[name, query] : FLOWS) {
    std::cout << std::endl;
    shards::make_view(&state, name, query);
  }
  std::cout << std::endl;
  ...
  for (std::string &insert : INSERTS) {
    std::cout << std::endl;
    shards::exec(&state, insert, &Callback, nullptr, nullptr);
  }
  std::cout << std::endl;
  ...
  for (std::string &select : QUERIES) {
    std::cout << std::endl;
    bool context = true;
    shards::exec(&state, select, &Callback, reinterpret_cast<void *>(&context),
                 nullptr);
  }
  std::cout << std::endl;
