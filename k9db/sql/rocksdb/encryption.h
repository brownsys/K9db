#ifndef K9DB_SQL_ROCKSDB_ENCRYPTION_H_
#define K9DB_SQL_ROCKSDB_ENCRYPTION_H_
...
class EncryptionManager {
 public:
  ...
  // Given our K9dbPrefixTransform, the seek prefix is the shard name.
...
class K9dbPrefixTransform : public rocksdb::SliceTransform {
 public:
  K9dbPrefixTransform() = default;

  const char *Name() const override { return "K9dbPrefix"; }
...
template <>
struct hash<k9db::sql::rocks::Cipher> {
  size_t operator()(const k9db::sql::rocks::Cipher &o) const {
    return hash<std::string>{}(o.cipher_);
  }
};

template <>
struct hash<k9db::sql::rocks::EncryptedKey> {
  size_t operator()(const k9db::sql::rocks::EncryptedKey &o) const {
...
#endif  // K9DB_SQL_ROCKSDB_ENCRYPTION_H_
