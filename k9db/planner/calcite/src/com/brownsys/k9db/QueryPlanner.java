...
import com.brownsys.k9db.rules.FilterPastJoin;
import com.brownsys.k9db.schema.K9dbSchemaFactory;
...
public class QueryPlanner {
  ...
  public QueryPlanner(CalciteSchema schema) {
    ...
    // When a subprogram is executed, its rules are executed one at a time in order, a rule is
    // repeated until its execution does not change the expression, before moving to the next
    // rule.
  ...
  // This is the entry function that is invoked from k9db c++.
  ...
  public static void plan(long ptr1, long ptr2, String query, boolean debugLogging)
    ...
    CalciteSchema schema = new K9dbSchemaFactory(generator).createSchema();
    ...
    // Extract k9db operators from plan and populate them into the c++ DataFlowGraph.
