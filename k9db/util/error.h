#ifndef K9DB_UTIL_ERROR_H_
#define K9DB_UTIL_ERROR_H_
...
class Error {
 public:
  // Error codes for the MySQL protocol.
  ...
  // Accessors.
  Code code() const { return this->code_; }
...
// Similar to absl::StatusOr but for our Error.
template <typename T>
class ErrorOr {
 public:
  // Constructors.
  ErrorOr(Error::Code code, const std::string &msg) : data_(Error(code, msg)) {}
  ErrorOr(Error::Code code, std::string &&msg)
      : data_(Error(code, std::move(msg))) {}

  explicit ErrorOr(const T &t) : data_(t) {}
  explicit ErrorOr(T &&t) : data_(std::move(t)) {}

  // Access.
  bool ok() const { return this->data_.index() == 1; }
  Error::Code &error() { return std::get<0>(this->data_); }
  T &result() { return std::get<1>(this->data_); }
