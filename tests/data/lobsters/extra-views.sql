...
CREATE VIEW q15 AS '"SELECT read_ribbons.* FROM read_ribbons WHERE read_ribbons.OWNER_user_id = ? AND read_ribbons.story_id = ?"';
CREATE VIEW q18 AS '"SELECT hidden_stories.story_id, hidden_stories.OWNER_user_id FROM hidden_stories WHERE hidden_stories.OWNER_user_id = ?"';
CREATE VIEW q19 AS '"SELECT users.* FROM users WHERE users.PII_username = ?"';
CREATE VIEW q20 AS '"SELECT hidden_stories.* FROM hidden_stories WHERE hidden_stories.OWNER_user_id = ? AND hidden_stories.story_id = ?"';
...
CREATE VIEW q24 AS '"SELECT saved_stories.* FROM saved_stories WHERE saved_stories.OWNER_user_id = ? AND saved_stories.story_id = ?"';
CREATE VIEW q31 AS '"SELECT 1, OWNER_user_id, story_id FROM hidden_stories WHERE OWNER_user_id = ? AND hidden_stories.story_id = ?"';
