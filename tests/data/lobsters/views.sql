...
CREATE VIEW q36 AS \
	'"SELECT read_ribbons.OWNER_user_id, COUNT(*) \
	FROM read_ribbons \
	...
	AND comments.user_id <> read_ribbons.OWNER_user_id \
	...
	AND ( \
     ( \
            parent_comments.user_id = read_ribbons.OWNER_user_id \
            AND \
     ...
     ( \
            ...
            AND \
            stories.user_id = read_ribbons.OWNER_user_id \
     ) \
     ) GROUP BY read_ribbons.OWNER_user_id"';
